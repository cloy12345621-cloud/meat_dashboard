...
# ==============================================================================
# 0. API 인증키 설정 (반드시 본인의 디코딩된 키를 입력하세요!)
# ==============================================================================
...
# ==============================================================================
# 2. 고도화된 데이터 정제 엔진 (컬럼 불일치 완전 방어)
# ==============================================================================
def auto_detect_numeric_col(df, possible_names, new_col_name='AMOUNT'):
    """API가 도축 수량 컬럼명을 무작위로 변경해도 안전하게 찾아 숫자로 변환합니다."""
...
def find_actual_col(df, possible_names):
    """텍스트 컬럼(도축장명, 축종명 등)을 유연하게 찾아냅니다."""
    col_map = {c.upper(): c for c in df.columns}
    for p_name in possible_names:
...
def fetch_and_merge_data():
    """농식품부(실적) + 행안부(인프라) 통합 데이터 파이프라인"""
    
    # 1. 농식품부 시도별 실적 (2번 API)
    ...
    except: df_sido = pd.DataFrame()

    # 2. 농식품부 도축장별 실적 (3번 API)
    ...
    try:
        ...
        if bpl_col: df_factory['join_key'] = df_factory[bpl_col].astype(str).str.replace(" ", "", regex=True)
    except: df_factory = pd.DataFrame()

    # 3. 행안부 도축업 인프라 (4번 API)
    ...
    try:
        ...
        df_house = pd.DataFrame(res.get('slaughterhouses', res.get('row', [])))
        if not df_house.empty:
            name_col = find_actual_col(df_house, ['bplNm', 'BPL_NM'])
            if name_col: df_house['join_key'] = df_house[name_col].astype(str).str.replace(" ", "", regex=True)
    ...
    if not df_factory.empty and not df_house.empty and 'join_key' in df_factory.columns and 'join_key' in df_house.columns:
        ...
        house_names = df_house['join_key'].tolist()
        
        # 이름이 60% 이상 유사하면 강제 결합 (예: 한강축산 -> 한강축산물종합처리장)
        valid_factory['match_key'] = valid_factory['join_key'].apply(lambda x: get_close_matches(x, house_names, n=1, cutoff=0.6))
    ...
    return df_sido, df_factory, df_master

def verify_grade_confirm(issue_no):
    """1번 API: 축산물 등급판정 확인서 XML 파싱"""
...
# 3. 글로벌 사이드바 및 필터 로직
# ==============================================================================
df_sido, df_factory, df_master = fetch_and_merge_data()
...
with st.sidebar:
    ...
    if not df_sido.empty and region_col:
        ...
        selected_sido = []
        st.warning("API 데이터 통신 대기 중입니다...")
    st.markdown("---")

# ==============================================================================
# 4. 메인 대시보드 및 시각화 (3단 탭)
# ==============================================================================
st.markdown("<div class='main-title'>Livestock Data Intelligence</div>", unsafe_allow_html=True)
st.markdown("<p style='color: #94A3B8; font-size: 1.1rem; margin-bottom: 2rem;'>선택 지역 기반 도축장 랭킹 및 실시간 융합 플랫폼</p>", unsafe_allow_html=True)
...
with tab1:
    if selected_sido and region_col:
        ...
        with col1:
            st.markdown(f"<div class='metric-card'><div class='metric-title'>선택 지역 총 도축량</div><div class='metric-value'>{int(total_slaughter):,}<span class='metric-unit'>두</span></div></div>", unsafe_allow_html=True)
        with col2:
...
with tab2:
    ...
    if not df_master.empty:
        ...
        st.success(f"✅ 농식품부(실적)와 행안부(주소)가 성공적으로 병합되었습니다. (총 {len(df_display)}건)")
        st.dataframe(df_display, use_container_width=True)
    else:
        st.warning("데이터 병합을 진행 중이거나, 양측 데이터의 기준 이름이 달라 결합되지 못했습니다.")
...
with tab3:
    ...
    if search_triggered and cert_number:
        grade_info = verify_grade_confirm(cert_number)
        if grade_info:
            ...
            if not df_master.empty and target_abatt:
                # 역추적을 위한 퍼지 탐색
                master_names = df_master['join_key_y'].tolist() if 'join_key_y' in df_master.columns else df_master['join_key'].tolist()
                closest_match = get_close_matches(target_abatt, master_names, n=1, cutoff=0.5)
                
                if closest_match:
                    trace_result = df_master[df_master['join_key_y' if 'join_key_y' in df_master else 'join_key'] == closest_match[0]]
                ...
                else: st.warning("도축장명이 행안부 인프라 DB에 매칭되지 않습니다.")
